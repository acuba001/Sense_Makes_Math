...
class YouTube(Resource):#, XApiController
    """
    """
    ...
    def isValidEndpoint(endpoint):
        # [WIP] Split the 'endpoint' with .split("/") and then validate all parts
        # against some regex template
        return True
...
def getAllYouTubeVideos():
    ...
    try:
        ...
        print(len(allYouTubePlaylistItems))


        # Grab all 'YouTube' videoResources by 'videoId'
        for playlistItem in allYouTubePlaylistItems_sorted:
            Options = {'parts':["id", "snippet"], 'id': playlistItem["snippet"]["resourceId"]["videoId"]}
